...
export function runCognitiveLearnerV8() {
  ...
  if (adaptiveSharpe > 1.2 && adaptiveMDD < 0.05) trendInference = "Alcista Estable";
  else if (adaptiveSharpe < 0.5 && adaptiveMDD > 0.2) trendInference = "Bajista Inestable";
  else if (adaptiveRobust > 85) trendInference = "Alta Resiliencia";

  // --- Inferencia contextual (mini red simbólica) ---
  let insight = "Estabilidad cognitiva detectada.";
  if (adaptiveSharpe > 1 && adaptiveRobust > 80) insight = "Alta consistencia intertemporal.";
  else if (adaptiveSharpe < 0) insight = "Desviación negativa detectada. Ajuste recomendado.";
  else if (adaptiveMDD > 0.2) insight = "Riesgo operacional elevado.";
  ...
  if (!fs.existsSync(path.dirname(CHART_PATH))) fs.mkdirSync(path.dirname(CHART_PATH), { recursive: true });
  fs.writeFileSync(
    CHART_PATH,
    JSON.stringify(
      {
