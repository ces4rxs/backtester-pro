...
function computeRiskMetrics(equities) {
    if (equities.length === 0)
        return { VaR_95: 0, CVaR_95: 0, MaxDrawdown: 0, TailRatio: 0 };
...
// ✅ Gráficos Plotly
async function generatePlots(data, outDir) {
...
async function main() {
    ...
    const key = Object.keys(first).find(k => k.includes("equity") || k.includes("balance") || k.includes("final"));
    if (!key) {
