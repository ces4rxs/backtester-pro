...
function safeLoadJSON(filePath: string): any | null {
  try {
...
function generateSystemReflection(data: MemorySummary): string {
  if (!data) return "Sin datos suficientes para análisis cognitivo.";
...
export function generateCognitiveDashboard() {
  ...
  const extract = (obj: any): MemorySummary => ({
