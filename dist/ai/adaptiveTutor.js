...
export function generateUnifiedAdvice(strategyId) {
    ...
    const sharpe = entry.Sharpe ?? entry.sharpe ?? entry.sharpeRatio ?? 0;
    const mdd = entry.MaxDD ?? entry.maxDD ?? entry.drawdown ?? entry.MaxDrawdown ?? 0;
    const cagr = entry.CAGR ?? entry.cagr ?? entry.growth ?? entry.ReturnCAGR ?? 0;
    const equity = entry.EquityFinal ?? entry.equityFinal ?? entry.Equity ?? 0;
