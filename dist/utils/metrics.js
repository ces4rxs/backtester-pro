 ...
 */
export function calculateMetrics(equityCurve, trades, totalBars, initialCash) {
    ...
    const std = Math.sqrt(returns.map(r => Math.pow(r - mean, 2)).reduce((a, b) => a + b, 0) /
        (returns.length - 1)) || 0;
    // Sharpe
    const sharpe = std > 0 ? (mean / std) * Math.sqrt(252) : 0;
    // Sortino
    const neg = returns.filter(r => r < 0);
    const downside = Math.sqrt(neg.map(r => r * r).reduce((a, b) => a + b, 0) / (neg.length || 1)) || 0;
