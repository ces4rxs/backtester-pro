...
// === Estado global ===
let backend: "pg" | "sqlite" | "jsonl" = "jsonl";
...
export async function saveSnapshot(snapshot: Snapshot) {
  ...
  if (backend === "pg" && pool) {
    const client = await pool.connect();
    await client.query(
      "INSERT INTO omega_market_snapshots (asset, price, source, latency_ms, ts) VALUES ($1, $2, $3, $4, NOW())",
      [snapshot.asset, snapshot.price, snapshot.source, snapshot.latency_ms ?? 0]
    );
    client.release();
  } else if (backend === "sqlite" && db) {
    db.prepare(
      "INSERT INTO omega_market_snapshots (asset, price, source, latency_ms, ts) VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)"
    ).run(
      snapshot.asset,
      snapshot.price,
      snapshot.source,
