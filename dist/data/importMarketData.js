...
function normalizeBars(data, source) {
    if (source === "binance") {
        return data.map((d) => ({
            t: Number(d[0]),
            o: Number(d[1]),
...
// === Lógica principal ===
export async function loadMarketData(asset) {
    try {
        switch (asset) {
            // Bitcoin — Binance
            case "BTCUSD": {
            ...
            case "XAUUSD": {
                ...
                try {
                    ...
                    if (apiKey) {
                        ...
                        if (!price)
                            throw new Error("Sin datos de Metals-API");
                        const now = Date.now();
                        const gold = [{ t: now, o: price, h: price, l: price, c: price }];
            ...
            case "XAGUSD": {
                ...
                if (!price)
                    throw new Error("Sin datos de plata");
                const now = Date.now();
            ...
            case "WTIUSD": {
                ...
                if (!price)
                    throw new Error("Sin datos de petróleo");
                const now = Date.now();
