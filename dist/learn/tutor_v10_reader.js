...
export function explainOptimizerReportConsole() {
    ...
    let riskComment = "";
    if (mdd < 0.01)
        riskComment = "riesgo extremadamente bajo (ideal para capital estable)";
    else if (mdd < 0.03)
        riskComment = "riesgo moderado con buena tolerancia";
    else
        riskComment = "riesgo elevado, requiere control de posición";
    let performanceComment = "";
    if (sharpe > 2)
        performanceComment = "rendimiento sobresaliente y estable";
    else if (sharpe > 1)
        performanceComment = "rendimiento sólido y consistente";
    else
        performanceComment = "rendimiento aún mejorable";
