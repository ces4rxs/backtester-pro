...
function stats(xs) {
    ...
    const pick = (q) => s[Math.min(s.length - 1, Math.max(0, Math.floor(q * (s.length - 1))))] ?? 0;
...
function consoleHist(series, title, bins = 20, width = 40) {
    if (!series.length)
        return;
...
function computeRScore(sharpeArr, mddArr, retArr) {
 ...
 */
export function monteCarloPlus(bars, strategy, baseOpts = { initialCash: 10000, feeBps: 10, slippageBps: 5 }, opts = {}) {
