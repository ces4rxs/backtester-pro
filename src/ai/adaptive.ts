...
async function parseCsvLastRow(file: string): Promise<AnyObj | null> {
  ...
  if (!headers || !lastValues) return null;
...
function normKeyMap(obj: AnyObj): AnyObj {
  const out: AnyObj = {};
...
function pickFeatures(row: AnyObj) {
  const r = normKeyMap(row);
  const features: AnyObj = {};
  const pick = (aliases: string[], name: string) => {
