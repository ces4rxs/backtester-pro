...
async function main() {
    ...
    const outputFile = path.join(process.cwd(), "reports", `OMEGA_Cognitive_Report_v14_${Date.now()}.pdf`);
    // --- Datos ---
    ...
    const docDefinition = {
        content: [
            ...
            ...(causes.length
                ? [{ text: "🧩 Causas identificadas:", bold: true }, ...causes.map((c) => `• ${c}`)]
                : []),
            ...(recs.length
                ? [{ text: "\n🔧 Recomendaciones técnicas:", bold: true }, ...recs.map((r) => `• ${r}`)]
                : []),
