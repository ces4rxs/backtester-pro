...
import { appendSample, loadMemory, saveMemory, type LearnSample, type LearnMemory } from "./memoryStore.js";
...
export function analyzeReports(): LearnSample[] {
...
export function consolidateMemoryV6(): LearnMemory {
...
function computeStats(samples: LearnSample[]) {
  const validSharpe = samples.map(s => s.sharpe).filter(Boolean) as number[];
  const validMdd = samples.map(s => s.mdd).filter(Boolean) as number[];
  const validConf = samples.map(s => s.robustnessProb).filter(Boolean) as number[];
