...
export function generateQuantumRiskV13(strategyId) {
    ...
    const baseRisk = 0.35 * vol +
        0.30 * tailRisk +
        0.15 * liquidityStress +
        0.20 * overfitPenalty;
    const buffers = 0.25 * stabilityBuff +
        0.20 * growthBuff +
    ...
    const controls = [];
    if (tier === "CRITICAL") {
        controls.push("Reducir exposición y activar stop de portafolio.", "Ejecutar Monte Carlo v4.4 con estrés de colas.", "Reentrenar con v12 (RETRAIN) y validar CVaR externo.");
    }
    else if (tier === "HIGH") {
        controls.push("Aplicar límites de posición por volatilidad.", "Habilitar filtros de liquidez y spreads.", "Ejecutar optimización ligera (v5) con penalización por MDD.");
    }
    else if (tier === "MODERATE") {
        controls.push("Mantener hedges parciales (oro o USD).", "Monitorear drawdown rolling y activar alertas > -20%.");
    }
    else {
        controls.push("Mantener parámetros; revisar semanal.", "Validar estabilidad con dataset extendido.");
    }
    // 7) Estructura final
    return {
