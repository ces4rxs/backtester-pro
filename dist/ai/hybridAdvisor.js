...
// ===============================================================
export async function generateUnifiedAdviceHybridV10(strategyId) {
    ...
    if (flagOn && needsImprove) {
        try {
            ...
            const manifest = { name: strategyId, params: { shortPeriod: 10, longPeriod: 50 } };
            const goal = { kind: "max_predictedSharpe" };
            ...
            const top = (report.frontier || []).slice(0, 3).map((f) => ({
                id: f.id,
                name: f.name,
                params: f.params,
                scores: f.judge,
