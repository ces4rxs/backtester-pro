...
export async function generateCognitiveRiskV14(strategy, quantumRisk) {
    try {
        console.log("🧠 [V14] Cognitive Risk SAFE start…");
        // Extraemos el riskScore del v13 si existe
        const baseScore = Number(quantumRisk?.risk?.riskScore ?? quantumRisk?.riskScore ?? 62);
        // Ajuste cognitivo leve (no-cost)
        const adjusted = Math.min(Math.max(baseScore * 1.02, 0), 100);
        const level = adjusted >= 80 ? "ALTO" : adjusted >= 60 ? "MEDIO" : "BAJO";
