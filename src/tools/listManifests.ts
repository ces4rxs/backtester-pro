...
const strategyFilter = (opts["strategy"] as string)?.toLowerCase();
const sinceDate = opts["since"] ? new Date(opts["since"] as string) : null;
...
let filtered = 0;
let metricsSummary: { equity: number[]; sharpe: number[]; cagr: number[] } = {
  equity: [],
  sharpe: [],
  cagr: [],
};
...
for (const file of manifestFiles) {
  ...
  if (equity) metricsSummary.equity.push(equity);
  if (sharpe) metricsSummary.sharpe.push(sharpe);
  if (cagr) metricsSummary.cagr.push(cagr);
  ...
  console.log(
    `${sealStatus.padEnd(11)}| ${metaBadge.padEnd(11)}| ${file.padEnd(42)} | ${
      m.strategy ?? "—"
    } / ${equity.toFixed(2)} / ${m.timestamp}`
  );

  csvRows.push(
    `${file},"${m.strategy ?? ""}",${equity.toFixed(2)},${sharpe.toFixed(
      4
    )},${cagr.toFixed(4)},${m.timestamp},${okSeal},${metaOk},"${normalizeHash(
      m.integrityHash
    )}"`
  );
}
...
if (summaryMode) {
  ...
  console.log(
    `  • Promedio Equity: ${avg(metricsSummary.equity).toFixed(2)}\n  • Promedio Sharpe: ${avg(
      metricsSummary.sharpe
