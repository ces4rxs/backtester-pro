...
try {
  ...
  if (!fs.existsSync(aiPath)) throw new Error("No encontré ai_report.json. Ejecuta primero: npm run ai-opt");

  const ai = JSON.parse(fs.readFileSync(aiPath, "utf8"));
  if (!ai?.best?.params) throw new Error("ai_report.json no contiene 'best.params'");

  // fuerza short < long por si la IA dejó algo al límite
  const raw = ai.best.params as { shortWin: number; longWin: number };
