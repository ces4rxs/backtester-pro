...
try {
    ...
    const wf = walkForward(bars, (p) => smaCrossover(Math.max(2, Math.min(p.shortWin, p.longWin - 1)), Math.max(p.shortWin + 1, p.longWin)), space, { k: 4, trainRatio: 0.7, iters: 60 });
    // 🎲 5) Monte Carlo Robustness Test
    const mc = monteCarlo(bars, strat, { initialCash: 10000, feeBps: 10, slippageBps: 5 }, { runs: 200, feeVarPct: 0.3, slipVarPct: 0.3, priceJitterPct: 0.005 });
