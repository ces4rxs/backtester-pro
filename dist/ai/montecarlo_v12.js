...
export async function generateMontecarloV12(strategyId, cognitiveContext = {}, opts = {}) {
    try {
        // 🧠 Heurística de semillas “inteligentes” (derivada del contexto v14/v13)
        const riskScore = Number(cognitiveContext?.risk?.riskScore ?? cognitiveContext?.riskScore ?? 65);
        const focusFactor = Math.min(Math.max(riskScore / 100, 0.3), 0.95); // 0.3–0.95
        // Menos runs, más enfoque (SAFE)
        const MAX_RUNS = Math.min(Math.max(opts.maxRuns ?? Math.round(2500 * focusFactor), 800), 3000);
        console.log(`🌀 [V12] Reflexive SAFE start: ${strategyId} | focus=${focusFactor.toFixed(2)} | runs=${MAX_RUNS}`);
        const mc = runMonteCarlo(MAX_RUNS);
        // Narrativa corta (explicabilidad)
        const narrative = riskScore > 70
