...
try {
    ...
    if (bars.length < 100)
        throw new Error(`Datos insuficientes. Se cargaron ${bars.length}, se recomiendan +100.`);
    const strategyBuilder = (p) => rsiMeanRevert(p.rsiPeriod, p.overSold, p.overBought);
    const space = {
    ...
    const results = runAdaptiveBacktest(bars, strategyBuilder, space, {
