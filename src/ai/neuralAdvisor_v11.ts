...
function quantGrade(score: number): string {
  if (score >= 9) return "A+";
  ...
  try {
    ...
    for (const s of mem?.samples ?? []) {
      out.push({
        ...
        CAGR: safeNum((s as any).cvar ?? (s as any).cagr),
...
function readLatestManifest(): Rec | null {
  try {
...
export function generateNeuralAdvisorV11(strategyId: string) {
  ...
  if (dispersionSharpe > 0.4) insights.push("Dispersión notable entre memoria y proyección en Sharpe → revisar consistencia.");
  if (Math.abs(fMDD) > 0.12) insights.push("Riesgo elevado (MDD) → aplicar límites y stops discretizados.");
  if (antiOverfit < 0.4) insights.push("Pocas muestras válidas para robustez → ejecuta más validaciones Monte Carlo.");
  if (!insights.length) insights.push("Cohesión reflexiva estable entre historial y proyección.");
