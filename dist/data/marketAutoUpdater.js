...
const ENDPOINTS = {
    ...
    XAUUSD: async () => {
        try {
            ...
            const price = Array.isArray(lj)
                ...
                : null;
            if (price)
                return { symbol: "XAU/USD", price, source: "Metals.Live" };
    ...
    SP500: async () => {
        const r = await fetch("https://query1.finance.yahoo.com/v8/finance/chart/%5EGSPC?interval=1d");
        const j = await r.json();
...
async function saveSnapshotLocal(asset) {
    const start = Date.now();
    try {
        const data = await ENDPOINTS[asset]();
        const latency = Date.now() - start;
        if (!data.price)
            throw new Error("Precio inválido o sin datos");
